 ...
 */
const distributeROICommission = async (memberId, roiAmount, session = null, customDate = null, sourceRefNo = null) => {
  try {
    ...
    for (const upline of uplineSponsors) {
      ...
      const payout = new PayoutModel({
        ...
        // ✅ FIXED: ref_no must be unique per payout and level to avoid DB collision between Base and Add-on ROIs
        ref_no: `ROI-L-${upline.level}-${sourceRefNo || (memberId + '-' + today)}`,
        amount: commissionAmount,
        level: upline.level,
